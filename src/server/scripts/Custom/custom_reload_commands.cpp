...
Created by Loud21
Updated by Zeraax
https://www.ac-web.org/forums/showthread.php?233198-Better-Item-Reload-(Includes-full-Item-and-Creature-Template-reloads)
...
void BetterLoadItem(Player* player, std::vector<uint32> itemID)
{
	...
	do
	{
		...
		if (dbcitem)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct SoundOverrideSubclassID (%i), must be %i .", entry, itemTemplate->SoundOverrideSubclass, dbcitem->SoundOverrideSubclassID);
...
public:
	...
	static bool HandleBetterItemReload(ChatHandler * handler, const char * args)
	{
        ...
        */
        ...
        {
            handler->PSendSysMessage("[|cffFF0000Item Reloader|r]: item ID %u was not found in the database", atoi(itemID));
            return true;
        }
